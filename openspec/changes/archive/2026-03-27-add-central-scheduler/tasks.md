...
- [x] 1.1 Add `dragonmantank/cron-expression` to `apps/brama-core/composer.json` and run `composer update`
- [x] 1.2 Create migration `Version20260310000001.php` in `apps/brama-core/migrations/`:
...
- [x] 2.1 Create `apps/brama-core/src/Scheduler/ScheduledJobRepository.php` — DBAL-based repository:
...
- [x] 2.2 Create `apps/brama-core/src/Scheduler/CronExpressionHelper.php` — wrapper around `dragonmantank/cron-expression`:
  - `computeNextRun(string $cronExpression, string $timezone = 'UTC'): \DateTimeImmutable`
  - `isValid(string $cronExpression): bool`
- [x] 2.3 Create `apps/brama-core/src/Scheduler/SchedulerService.php` — orchestration logic:
...
- [x] 3.1 Create `apps/brama-core/src/Command/SchedulerRunCommand.php`:
...
- [x] 5.1 Create `apps/brama-core/src/Controller/Admin/SchedulerController.php`:
...
- [x] 5.2 Create Twig template `apps/brama-core/templates/admin/scheduler/index.html.twig`
- [x] 5.3 Create `apps/brama-core/src/Controller/Api/Internal/SchedulerRunNowController.php`:
  - `POST /api/v1/internal/scheduler/{id}/run` — call `ScheduledJobRepository::triggerNow($id)`, return JSON
- [x] 5.4 Create `apps/brama-core/src/Controller/Api/Internal/SchedulerToggleController.php`:
