...
- [ ] 3.7 Verify: `make e2e-register-agents` succeeds with updated payloads
...
- [ ] 4.2 Verify: after `make e2e-prepare`, all agents have `health_status != 'unknown'` in DB

## 5. Verify E2E Tests Pass
- [ ] 5.1 Run `hello_agent_test.js` — verify `hello-agent is present and healthy after discovery` passes
- [ ] 5.2 Run `news_maker_admin_test.js` — verify `news-maker-agent is present and healthy after discovery` passes
- [ ] 5.3 Run full E2E suite — verify no regressions
...
- [ ] 7.4 `make e2e` — E2E suite passes (specifically health badge tests)
